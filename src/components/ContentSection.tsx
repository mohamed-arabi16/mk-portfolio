      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {isLoading ? (
            ...
            contentItems.map((item) => (
              ...
              >
                <div className="relative overflow-hidden rounded-t-[var(--radius-glass)]">
                  <img
                    src={item.thumbnail_url || 'https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=400&h=300&fit=crop'}
                  ...
                  {item.platform && (
                    <div className={`absolute top-3 left-3 px-2 py-1 rounded-md text-xs font-medium ${platformColors[item.platform as keyof typeof platformColors]}`}>
                      {item.platform}
                    </div>
                  )}
                  
                  {item.content_type && (
                    <div className={`absolute top-3 right-3 px-2 py-1 rounded-md text-xs font-medium ${typeColors[item.content_type as keyof typeof typeColors] || 'bg-muted text-muted-foreground'}`}>
            ...
            <div className="col-span-full text-center py-12">
              <p className="text-muted-foreground">No content yet. Add some in the admin panel!</p>
            </div>
          )}
        </div>
